...
def rgb_patch_plot(
    rgb_colors, color_names, title, filename, patch_gap=0.05, num_across=6
):
    """Draw a set of color patches, specified as linear rgb colors."""
...
def xyz_patch_plot(
    xyz_colors, color_names, title, filename, patch_gap=0.05, num_across=6
):
    """Draw a set of color patches specified as xyz colors."""
    rgb_colors = []
...
def spectrum_subplot(spectrum):
    ...
    pylab.plot(
        spectrum[:, 0], spectrum[:, 1], color="k", linewidth=2.0, antialiased=True
    )


def spectrum_plot(
    spectrum, title, filename, xlabel="Wavelength ($nm$)", ylabel="Intensity ($W/m^2$)"
):
    """Plot for a single spectrum -
    In a two part graph, plot:
    top: color of the spectrum, as a large patch.
